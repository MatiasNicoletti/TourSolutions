...
function getBlueMarkerIcon(num) {
    return 'newicons/black' + num + '.png';
}

function getRedMarkerIcon(num) {
    return 'newicons/red' + num + '.png';
}
...
function formatDirections(gdir, mode) {
    ...
    var retStr = "<table class='gebddir' border=0 cell-spacing=0 style:'font-family: 'Work Sans', sans-serif !important;'>\n";
    ...
    for (var i = 0; i < gdir.legs.length; ++i) {
        ...
        for (var j = 0; j < route.steps.length; ++j) {
            ...
            retStr += "\t<tr class='text'><td class='" + classStr + "'></td>"
                + "<td class='" + classStr + "' style='font-family: 'Work Sans', sans-serif !important;'>" + route.steps[j].instructions
...
function createTomTomLink(gdir) {
    ...
    var retStr = "<form method='POST' action='tomtom.php' target='tomTomIFrame'>\n";
...
function drawMarker(latlng, addr, label, num) {
    ...
    google.maps.event.addListener(marker, 'click', function(event) {
        ...
        var labelStr = (label == null) ? "" : "<b style='color:red !important'>" + label + "</b><br>";
...
function directions(m, walking, bicycling, avoidHighways, avoidTolls, metricUnits) {
    ga('send', 'event', 'app', 'calculate', (m == 1 ? 'az' : 'roundtrip'), tsp.getWaypoints().length);
...
function orderedDirections(walking, bicycling, avoidHighways, avoidTolls, metricUnits) {
    ga('send', 'event', 'app', 'calculate', 'ordered', tsp.getWaypoints().length);
    jQuery('#dialogProgress').dialog('open');
    tsp.setAvoidHighways(avoidHighways);
    tsp.setAvoidTolls(avoidTolls);
    if (metricUnits)
...
function onSolveCallback(myTsp) {
    ...
    console.log(dir.legs[0].start_address)
    console.log(dir.legs[0].end_address)
    // console.log(dir.legs[0].start_address.split(',')[0] + dir.legs[0].start_address.split(',')[1]+ dir.legs[0].start_address.split(',')[2])
    let flag = true;
    for (let index = 0; index < Object.keys({...localStorage}).length; index++) {
         const {duration, distance} = {...JSON.parse(localStorage.getItem(`trip-${index}`))};
        if(duration == formatTime(getTotalDuration(dir)) &&  distance == formatLength(getTotalDistance(dir))){
            flag = false;
            break;
        }
    ...
    if(flag){
        localStorage.setItem(`trip-${Object.keys({...localStorage}).length}`, 
        JSON.stringify({
            duration:formatTime(getTotalDuration(dir)), 
            distance:formatLength(getTotalDistance(dir)), 
            date: new Date(),
            start: dir.legs[0].start_address,
            end: dir.legs[0].end_address
        }))
    }
    
    
    var pathStr = "<p class='result-p'>Duración del recorrido: <strong>" + formatTime(getTotalDuration(dir)) + "</strong></p>";
    pathStr += "<p class='result-p' style='margin-bottom: 2rem !important'> Distancia del recorrido: <strong>" + formatLength(getTotalDistance(dir)) +
        " (" + formatLengthMiles(getTotalDistance(dir)) + ")</strong></p>";
